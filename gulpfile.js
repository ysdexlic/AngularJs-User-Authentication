...
gulp.task('build', ['clean'], function(cb) {
  runSequence(['jshint', 'sass', 'templates'], cb);
});
...
gulp.task('test:unit', function(cb) {
  runSequence(['build', 'karma'], function() {
    process.exit(0);
  });
});
...
//run the server after having built generated files, and watch for changes
gulp.task('serve:tdd', ['build', 'jshint'], function() {
  browserSync({
    port: config.port,
    ui: {
      port: config.uiPort
  ...
  gulp.watch(config.html, reload);
  gulp.watch(config.scss, ['sass', 'reload']);
  gulp.watch(config.js, ['reload', 'jshint', 'karma']);
  gulp.watch(config.tpl, ['templates', 'reload', 'karma']);
  gulp.watch(config.assets, ['reload', 'karma']);
});
